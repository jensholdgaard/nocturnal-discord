...
//
// The Discord door to `profiles::upload_export`; the site's drop zone is the
// other. Bags, bank and coin are dropped at parse time.
...
pub async fn upload(
    ...
    let outcome = crate::profiles::upload_export(
        &ctx.data().driver,
        ledger_guild,
        player,
        &file.filename,
